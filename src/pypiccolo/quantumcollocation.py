...
class QuantumSystem:
    def __init__(
        ...
        **kwargs,
        ...
        self.value = get_global(mod_piccolo, 'QuantumSystem')(*self.args)
...
class QuantumControlProblem:
    def solve(self, max_iter: int | None = None, **kwargs) -> None:
        ...
        if max_iter is not None:
            fn = get_global(mod_piccolo, 'solve!')
            args = [self.value,]
            names = ['max_iter']
...
def plot_unitary_populations(problem: QuantumControlProblem, display_plot: bool = False):
    ...
    if display_plot == True:
        display = get_global(mod_piccolo, 'display')(plot)
    return plot
